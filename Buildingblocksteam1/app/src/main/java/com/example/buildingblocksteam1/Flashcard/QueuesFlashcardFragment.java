...
public class QueuesFlashcardFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        flashcards.add(new Flashcards("What is the difference between an array and a linked list?", "An array has a fixed size and stores its elements in contiguous memory locations, whereas a linked list can queuesally grow or shrink and stores its elements in non-contiguous memory locations."));
        ...
        flashcards.add(new Flashcards("What are some advantages of using a linked list over an array?", "Linked lists can queuesally grow or shrink, insert or delete elements at constant time complexity (in certain cases), and use only the memory required for their elements (unlike queues, which may have unused memory if their size is larger than the number of elements they store)."));
        ...
        flashcards.add(new Flashcards("What is the time complexity of queues for an element in a linked list?", "O(n), where n is the number of nodes in the list. This is because each node must be checked in order until the desired element is found (or the end of the list is reached)."));
